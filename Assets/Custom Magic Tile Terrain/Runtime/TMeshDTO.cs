...
namespace CustomMagicSandbox.TileTerrain
{
  ...
  public class TMeshDTO
  {
    ...
    [SerializeField] internal List<int> _gridIndexToUV = new List<int>(); //Grid index points into the UV list. 

    //Naming like in Unity Mesh - UV1 is often called UV0
    [SerializeField] internal List<Vector2> _uv1 = new List<Vector2>(); //Main Texture UV
    [SerializeField] internal List<Vector2> _uv2 = new List<Vector2>();//Baked Lightmaps
    [SerializeField] internal List<Vector2> _uv3 = new List<Vector2>(); //Baked GI
    ...
    public void Create(Vector2Int dimensions, float tileSize)
    {
      ...
      //Initialize the TMesh with its grid positions.
      CreateGridMesh();
    }

    void CreateGridMesh()
    {
      int _gridPointCount = (_sizeX + 1) * (_sizeZ + 1);
      ...
      for (int z = 0, i = 0; z <= _sizeZ; z++) //Z axis
      {
        for (int x = 0; x <= _sizeX; x++, i++) //X axis
        {
          ...
          if (i == 5) continue;
          _gridPoints[i] = new Vector3((float)x * _tileSize, 0, (float)z * _tileSize);
        }
      }
